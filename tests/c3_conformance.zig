...
const backend = syntax.languages.c3.backend;
...
test "C3 backend conforms" {
    ...
    try conformance.expectConforms(backend, .{
        .valid = .{ .source = @embedFile("corpus/c3/complete.txt"), .required_scopes = &.{ .keyword, .namespace, .type, .function, .parameter, .property, .variable, .string, .escape, .number, .boolean, .comment } },
        .malformed = .{ .source = "module broken::name;\nstruct Open {\n String field;\nfn int run(int value {\n", .required_scopes = &.{ .keyword, .namespace, .type, .property, .function, .parameter } },
        .multiline = .{ .source = "module demo;\nfn int one(int value) { return value; }\n", .required_scopes = &.{ .namespace, .function, .parameter } },
        .escapable = .{ .source = "String value = \"<&>\\q'\"; // comment", .required_scopes = &.{ .type, .variable, .string, .escape, .comment } },
    });
}

test "C3 parser classifies modules structs functions fields and qualified calls" {
    const source = @embedFile("corpus/c3/complete.txt");
    ...
    try expect(source, sink.captures(), "printfn", .function);
    try expect(source, sink.captures(), "LIMIT", .variable);
}
